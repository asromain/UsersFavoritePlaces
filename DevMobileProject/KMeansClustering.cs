...
namespace DevMobileProject
{
    class KMeansClustering
    {
        ...
        public KMeansClustering(int nbClusters, double[][] rawData)
        {
        ...
        public String displayResult()
        {
            ...
            result += "Raw unclustered data:\n";
            result += "   Height Weight";
            result += "\n-------------------\n";
            result += ShowData(rawData, 5, true, true);

            result += "\nNombre de clusters : " + nbClusters + "\n";

            int[] clustering = Cluster();
            
            result += "\nK-means clustering complete\n";
            result += "Final clustering in internal form :\n\n";
            result += ShowVector(clustering, true);
        
            result += "Raw data by cluster:\n";
            result += ShowClustered(rawData, clustering, nbClusters, 5);
        ...
        public List<List<Double[]>> getCalculatedClusters()
        {
            if (clusters == null)
            {
                calculateClusters();
        ...
        public void calculateClusters()
        {
            ...
            clusters = getClusters(rawData, clustering, nbClusters);
        }
        ...
        public int[] Cluster()
        {
            ...
            //    update means
            // end loop
            int[] clustering = InitClustering(data.Length, nbClusters, 0); // semi-random initialization
            double[][] means = Allocate(nbClusters, data[0].Length); // small convenience
        ...
        public String ShowData(double[][] rawData, int decimals, bool indices, bool newLine)
        {
            String result = "";
            for (int i = 0; i < rawData.Length; ++i)
            {
                ...
                for (int j = 0; j < rawData[i].Length; ++j)
                {
                    ...
                    result += rawData[i][j].ToString("F" + decimals) + " ";
                }
                result += "\n";
            }
            if (newLine) result += "\n";
        ...
        public String ShowClustered(double[][] data, int[] clustering, int numClusters, int decimals)
        {
            String result = "";
            for (int k = 0; k < numClusters; ++k)
            {
        ...
        public List<List<double[]>> getClusters(double[][] data, int[] clustering, int numClusters)
        {
            ...
            for (int k = 0; k < numClusters; ++k)
            {
                ...
                for (int i = 0; i < data.Length; ++i)
                {
                    int clusterID = clustering[i];
                    if (clusterID != k) continue;
                    double[] clusterData = new double[2];
                    for (int j = 0; j < data[i].Length; ++j)
                    {
                        //if (data[i][j] >= 0.0) result += " ";
                        //result += data[i][j].ToString("F" + decimals) + " ";
                        clusterData[j] = data[i][j];
                    }
                    //result += "\n";
